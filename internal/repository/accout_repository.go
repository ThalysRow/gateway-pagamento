...
import (
	...
	"github.com/ThalysRow/imersao-22/go-api/internal/domain"
)
...
func (r *AccountRepository) FindByID(id string) (*domain.Account, error) {
	...
	err := r.db.QueryRow("SELECT id, name, email, api_key, balance, created_at, updated_at FROM accounts WHERE id = $1",
...
func (r *AccountRepository) FindByApiKey(apiKey string) (*domain.Account, error) {
	...
	err := r.db.QueryRow("SELECT id, name, email, api_key, balance, created_at, updated_at FROM accounts WHERE api_key = $1",
...
func (r *AccountRepository) UpdateBalance(account *domain.Account) error {
	...
	account.Balance = currentBalance + account.Balance

	_, err = tx.Exec("UPDATE accounts SET balance = $1, updated_at = $2 WHERE id = $3",
		account.Balance+account.Balance, time.Now(), account.ID)
	if err != nil {
		return err
	}
